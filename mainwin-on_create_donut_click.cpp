...
{ 
    ...
    for (auto s: filling_string) c_filling.append(s);
    ...
    if (frosting < 0 || frosting >= frosting_string.size()) frosting = Unfrosted;

    Filling filling = (Filling)(c_filling.get_active_row_number());
    if (filling < 0 || filling >= filling_string.size()) filling = Unfilled;
